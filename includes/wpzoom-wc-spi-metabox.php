...
if ( ! class_exists( 'WPZOOM_WC_Secondary_Image_Metabox' ) ) {
	...
	class WPZOOM_WC_Secondary_Image_Metabox {
		...
		public function thumbnail_meta_box() {
			...
			// Not escaped on output: the markup is generated by this class and
			// includes the media modal bootstrap script.
			echo $this->post_thumbnail_html( $thumbnail_id ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		...
		public function enqueue_admin_scripts( $hook ) {
			...
			// and only for the post type the thumbnail was registered for
			$screen = function_exists( 'get_current_screen' ) ? get_current_screen() : null;

			if ( $screen && $screen->post_type && $screen->post_type !== $this->post_type )
				return;
			...
			if ( version_compare( $wp_version, '3.5', '<' ) ) {	
				...
				// Both stay in the header: the metabox markup bootstraps MediaModal inline.
				wp_enqueue_script( "wpzoom-wc-spi-metabox", $this->plugins_url( 'assets/js/wpzoom-wc-spi-metabox.js', __FILE__ ), array( 'jquery', 'wp-i18n', 'set-post-thumbnail' ), WPZOOM_WC_SPI_VER, false );
				wp_enqueue_script( "wpzoom-wc-spi-metabox-modal", $this->plugins_url( 'assets/js/media-modal.js', __FILE__ ), array( 'jquery', 'media-models' ), WPZOOM_WC_SPI_VER, false );
			}

			wp_enqueue_style( "wpzoom-wc-spi-metabox-css", $this->plugins_url( 'assets/css/wpzoom-wc-spi-metabox.css', __FILE__ ), array(), WPZOOM_WC_SPI_VER );
		}
		
		public function admin_header_scripts() {
			// Cast to int so a missing ID can't produce invalid JavaScript.
		...
		public function action_delete_attachment($post_id) {
			// Deletes across all posts and keeps the meta cache in sync, unlike a
			// direct $wpdb query.
			delete_metadata( 'post', 0, $this->get_meta_key(), $post_id, true );
		}
